...
func (pi *ProjectInfo) convertStrToSlices() {
	pi.Owners = strToSlice(pi.OwnersStr)
	pi.Logs = strToSlice(pi.LogsStr)
	pi.Stats = strToSlice(pi.StatsStr)
}
...
//Converts strings separated by comma's into a string slice
func strToSlice(s string) []string {
	slice := strings.Split(s, ",")
	rv := []string{}
	for i := 0; i < len(slice); i++ {
		if v := strings.TrimSpace(slice[i]); v != "" {
			rv = append(rv, v)
		}
	}
	return rv
